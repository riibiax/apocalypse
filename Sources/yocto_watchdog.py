...
class YWatchdog(YFunction):
    ...
    The watchdog can be driven direcly with <i>pulse</i> and <i>delayedpulse</i> methode to switch
    ...
    def get_state(self):
        ...
        @return either YWatchdog.STATE_A or YWatchdog.STATE_B, according to the state of the relay (A for
    ...
    def set_state(self, newval):
        ...
        @param newval : either YWatchdog.STATE_A or YWatchdog.STATE_B, according to the state of the relay
    ...
    def set_output(self, newval):
        ...
        of the relay, when used as a simple switch (single throw)
    ...
    def get_pulseTimer(self):
        ...
        @return an integer corresponding to the number of milliseconds remaining before the relay is
    ...
    def get_delayedPulseTimer(self):
        ...
        @return an object (aggregate) corresponding to the number of milliseconds remaining before a pulse
        (delayedPulse() call)
                When there is no scheduled pulse, returns zero
        
        On failure, throws an exception or returns YWatchdog.DELAYEDPULSETIMER_INVALID.
        """
        if self._cacheExpiration <= YAPI.GetTickCount():
            if YAPI.YISERR(self.load(YAPI.DefaultCacheValidity)):
    ...
    def get_autoStart(self):
        """
        Return the watchdog runing state at module power up
        
        @return either YWatchdog.AUTOSTART_OFF or YWatchdog.AUTOSTART_ON
    ...
    def get_running(self):
        """
        Return the watchdog running state
        
        @return either YWatchdog.RUNNING_OFF or YWatchdog.RUNNING_ON
    ...
    def set_running(self, newval):
        """
        Allow to manually start / stop the watchog
        
        @param newval : either YWatchdog.RUNNING_OFF or YWatchdog.RUNNING_ON
