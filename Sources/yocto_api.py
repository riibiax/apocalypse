...
def YByte2AnsiPython2x(binBuffer):
    return str(binBuffer)

def YSetBufferLenPython2x(binBuffer,l):
    binBuffer[l]=chr(0)

def YByte2AnsiPython3x(binBuffer):
    return str(binBuffer,"ASCII")

def YSetBufferLenPython3x(binBuffer,l):
    binBuffer[l]=0


if sys.version_info < (3, 0):
    YByte2Ansi = YByte2AnsiPython2x
    YSetBufferLen = YSetBufferLenPython2x
else:
    YByte2Ansi = YByte2AnsiPython3x
    YSetBufferLen = YSetBufferLenPython3x
...
class YAPI:
    ...
    class TJsonParser:
        ...
        def __init__(self,jsonData):
            ...
            jsonData = jsonData[p1 + 4: len(jsonData)]
    ...
    def yloadYapiCDLL():
        ...
        #
            ...
            if machine.find("arm")>=0:
                ...
                YAPI._yApiCLibFileFallback = libpath+"/cdll/libyapi-armhf.so"
    ...
    def Sleep(ms_duration, errmsgRef = None):
        ...
        while ok:
            ...
            if YAPI.GetTickCount() < timeout:
                ...
                if YAPI.YISERR(res):
                    if not errmsgRef is None: errmsgRef.value =  YByte2Ansi(errBuffer.value)
                    return res
            ok= YAPI.GetTickCount() < timeout
        if errmsgRef is not None: errmsgRef.value =  YByte2Ansi(errBuffer.value)
        return res
    ...
    @staticmethod
    def _parseCalibParams(calibParams,calibOffset,resolution, calTypeRef, calhdlRef,  iparams,rawPt,calPt):

        p=calibParams.find(',')
        if p<=0: return False

        calTypeRef.value = int(calibParams[0:p])
        calhdlRef.value = YAPI._getCalibrationHandler(calTypeRef.value)
        if calhdlRef.value is None :  return False
        p+=1
        i=p
        cnt=0

        YAPI._setArrayLength(iparams,20)
        YAPI._setArrayLength(rawPt,10)
        YAPI._setArrayLength(calPt,10)

        while i<len(calibParams)-1 and calibParams[i]!=',':
            i+=1
        while i>p:
            iparams[cnt]=int(calibParams[p:i])

            if cnt % 2 == 0:
                rawPt[int(cnt/ 2) ] =  (iparams[cnt] + calibOffset) * resolution
            else:
                calPt[int(cnt / 2) ] =  (iparams[cnt] + calibOffset) * resolution
        ...
        YAPI._setArrayLength(iparams,cnt)
        YAPI._setArrayLength(rawPt,int(cnt/2))
        YAPI._setArrayLength(calPt,int(cnt/2))

        return True

    @staticmethod
    def _decodeCalibrationPoints(calibParams,rawPt,calPt,resolution,calibOffset):
        calhdlRef  = YRefParam()
        caltypeRef = YRefParam()
        iparams = ()
        if YAPI._parseCalibParams(calibParams, calibOffset, resolution, caltypeRef, calhdlRef,  iparams,  rawPt,calPt):
            if caltypeRef.value > 10 : return YAPI.NOT_SUPPORTED
            del rawPt[caltypeRef.value:]
            del calPt[caltypeRef.value:]
            return YAPI.SUCCESS
    ...
    def _applyCalibration(rawValue,calibParams,calibOffset,resolution):
        ...
        if not YAPI._parseCalibParams(calibParams,calibOffset,resolution, calTypeRef, calhdlRef,  iparams, rawPt,calPt):
            return YAPI.INVALID_DOUBLE


        return calhdlRef.value(rawValue,calTypeRef.value,iparams,rawPt,calPt)
    ...
    def yapiUpdateDeviceList(force, errmsgRef=None):
        ...
        if YAPI.YISERR(res):
           if  not errmsgRef is None:  errmsgRef.value = YByte2Ansi(buffer.value)
        return res
    ...
    def native_yLogFunction(log,  loglen):
        ...
        if yLogFct is not  None:
            ...
            yLogFct(YByte2Ansi(log))
        return 0

    @staticmethod
    def RegisterLogFunction(logfun):
        """
        Register a log callback function. This callback will be called each time
    ...
    def yapiGetDeviceInfo(d, infos, errmsgRef=None):
        ...
        if  errmsgRef is not None:  errmsgRef.value =  YByte2Ansi(buffer.value)
        return res
    ...
    def native_yDeviceArrivalCallback(d):
        ...
        ev.modul = YModule.FindModule(YByte2Ansi(infos.serial) + ".module")
    ...
    def apiGetAPIVersion(versionRef, dateRef):
        ...
        versionRef.value = YByte2Ansi(pversion.buffer)
        dateRef.value = YByte2Ansi(pdate.buffer)
        return res
    ...
    def yapiGetFunctionInfo(fundesc, devdescRef, serialRef, funcIdRef, funcNameRef, funcValRef,errmsgRef =None):
        ...
        serialRef.value = YByte2Ansi(serialBuffer.value)
        funcIdRef.value = YByte2Ansi(funcIdBuffer.value)
        funcNameRef.value = YByte2Ansi(funcNameBuffer.value)
        funcValRef.value = YByte2Ansi(funcValBuffer.value)
        if  errmsgRef is not None:  errmsgRef.value = YByte2Ansi(errBuffer.value)
        return res
...
class YDevice:
    ...
    def _HTTPRequestPrepare(self,request):
        ...
        if not self._subpathinit:
            ...
            if YAPI.YISERR(res):
                return res, YByte2Ansi(errbuf.value)
            #noinspection PyUnresolvedReferences
            ...
            if YAPI.YISERR(res):
                return res, YByte2Ansi(errbuf.value)
            self._rootdevice = YByte2Ansi(root.value)
            self._subpath =YByte2Ansi(b.value)
            self._subpathinit = True

        p = request.find("/")
        newrequest = request[0:p]+self._subpath+request[p+1:]
    ...
    def HTTPRequestAsync(self,request, callback, context, errmsgRef = None):
        ...
        if YAPI.YISERR(res):
            ...
            return res
        res = YAPI._yapiHTTPRequestAsync(ctypes.create_string_buffer(self._rootdevice.encode("ASCII")),ctypes.create_string_buffer(newrequest.encode("ASCII")), None,None, errbuf)
        if YAPI.YISERR(res):
            if not errmsgRef is None: errmsgRef.value = YByte2Ansi(errbuf.value)
            return res
        return YAPI.SUCCESS
    ...
    def HTTPRequest(self,request, bufferRef, errmsgRef=None):
        ...
        newrequest_c = ctypes.create_string_buffer(newrequest.encode("ASCII"))
        reply_c = POINTER(ctypes.c_ubyte)()
        neededsize_c = ctypes.c_int(0)
        res = YAPI._yapiHTTPRequestSyncStart(iohdl,root_c,newrequest_c,ctypes.byref(reply_c),ctypes.byref(neededsize_c), errbuf)
...
class YFunction(object):
    ...
    def  _buildSetRequest(self,changeattr, changeval, requestRef, errmsgRef=None):
        ...
        if YAPI.YISERR(res):
            if not errmsgRef is None: errmsgRef.value = YByte2Ansi(errbuff.value)
            self._throw(res, errmsgRef.value)
            return res


        requestRef.value = "GET /api/" + YByte2Ansi(funcid.value) + "/"
        uchangeval = ""
    ...
    def _encodeCalibrationPoints(self,rawValues,refValues,resolution,calibrationOffset):
        minRaw=0
        cnt=0
        res=""
        npt= min(len(rawValues),len(refValues))
        for i in range(npt):
...
# --- (end of generated code: YModule implementation)
    ...
    def __str__(self):
        name = self.get_serialNumber()
        lname = self.get_logicalName()
        if lname != "" and lname!=YModule.LOGICALNAME_INVALID : name +=  " (" + lname + ")"
        return name

    def setImmutableAttributes(self,infosRef):
        self._serialNumber = YByte2Ansi(infosRef.serial)
        self._productName = YByte2Ansi(infosRef.productname)
