...
class YDataRun():
    ...
    def finalize(self):
        ...
        if self.isLive and self.startTimeUTC == 0:
            self.startTimeUTC = time.time() - self.duration
    ...
    def refresh(self):
        if self._isLive:
            ...
            if time.time() > self._startTimeUTC + self._duration:
    ...
    def computeValues(self,pos):
        ...
        thisAvail = floor(stream.get_startTime() / self._browseInterval)
        nextMissing = ceil((stream.get_startTime() + stream.get_rowCount() * streamInterval) / self._browseInterval)
        ...
        if si+1 >= len(self._streams):
            ...
            nextAvail = floor(nextStream.get_startTime() / self._browseInterval)
        ...
        if prevMissing < thisAvail or prevMissing == 0:
            ...
            pos = ceil(stream.get_startTime() / self._browseInterval)
            row = round((pos * self._browseInterval - stream.get_startTime()) / streamInterval)
        ...
        stopTime = ceil((stream.get_startTime() + stream.get_rowCount() * stream.get_dataSamplesInterval()) / self._browseInterval) * self._browseInterval
